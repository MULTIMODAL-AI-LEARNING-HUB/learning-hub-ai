"""Gemini API Key Rotator and Failover Management."""
...
class GeminiKeyRotator:
    _instance: Optional["GeminiKeyRotator"] = None
    ...
    @classmethod
    def get_instance(cls) -> "GeminiKeyRotator":
    ...
    def _load_from_settings(self):
        ...
        if raw:
            # Support comma-separated or whitespace-separated keys
            keys = [k.strip() for k in raw.replace(",", " ").split() if k.strip()]
            for i, k in enumerate(keys):
                self.add_key(k, key_name=f"Config Key {i+1}")
        logger.info("Initialized GeminiKeyRotator with %d keys from settings", len(self.keys))
    ...
    def sync_keys(self, keys_data: list[dict[str, Any]]):
        ...
        if new_keys:
            self.keys = new_keys
            self._current_idx = 0
            logger.info("Synced %d keys into GeminiKeyRotator", len(self.keys))

    def get_next_key(self) -> str:
        """Get the next available active Gemini API Key via Round-Robin with cooldown handling."""
        now = time.time()
        available = [k for k in self.keys if k.is_available]

        if not available:
            # If all active keys are on cooldown, pick the one whose cooldown expires soonest
            active_keys = [k for k in self.keys if k.is_active]
            if active_keys:
                logger.warning("All active keys in cooldown. Re-activating the soonest expiring key.")
                ...
                raise RuntimeError("No active Gemini API keys configured in pool.")

        # Round Robin selection
        self._current_idx = (self._current_idx + 1) % len(available)
        selected = available[self._current_idx]
    ...
    def report_rate_limit(self, key_str: str, cooldown_seconds: float = 60.0):
        ...
        for item in self.keys:
            if item.key == key_str.strip():
                ...
                logger.warning("Key %s placed in cooldown for %.0fs due to rate limit/error", item.masked_key, cooldown_seconds)
                break
